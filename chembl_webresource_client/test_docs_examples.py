...
class TestDocsExamples(unittest.TestCase):
    ...
    def test_search_molecule_by_synonym(self):
        ...
        self.assertTrue(all([any(['Sildenafil' in y['molecule_synonym'] for y in x['molecule_synonyms']]) for x in res]))
    ...
    def test_search_target_by_gene_name(self):
        ...
        self.assertEqual(len(res), 2)
        self.assertTrue(all([all([any(['brd4' in z['component_synonym'].lower() for z in y['target_component_synonyms']]) for y in x['target_components']]) for x in res]))
    ...
    def test_search_gene_name_in_target_synonym(self):
        ...
        self.assertTrue(all([any([any(['GABRB2' in z['component_synonym'].upper() for z in y['target_component_synonyms']]) for y in x['target_components']]) for x in res]))
    ...
    def test_chembl_id_to_human_gene_names(self):
        ...
        self.assertTrue({'ORF35', 'Pept2', 'Oat1'}.issubset(compounds2targets['CHEMBL819']), compounds2targets['CHEMBL819'])
        self.assertTrue({'B2AR', 'AVPR1', 'UBP41'}.issubset(compounds2targets['CHEMBL820']), compounds2targets['CHEMBL820'])
        self.assertTrue({'OATP1B3', 'OAT3', 'OCT2'}.issubset(compounds2targets['CHEMBL821']), compounds2targets['CHEMBL821'])
    ...
    def test_similarity_70(self):
        ...
        self.assertTrue({'CHEMBL163148', 'CHEMBL351485', 'CHEMBL2260706', 'CHEMBL1234172', 'CHEMBL1359634'}.issubset(set([r['molecule_chembl_id'] for r in res])))   
    ...
    def test_smiles_substructure(self):
        ...
        self.assertTrue({'CHEMBL442138', 'CHEMBL287955', 'CHEMBL38899', 'CHEMBL53821', 'CHEMBL55826'}.issubset(set([r['molecule_chembl_id'] for r in res])))
    ...
    def test_chembl_id_substructure(self):
        ...
        self.assertTrue({'CHEMBL25', 'CHEMBL7666', 'CHEMBL10222', 'CHEMBL10008'}.issubset(set([r['molecule_chembl_id'] for r in res])))
    ...
    def test_get_multiple_by_smiles(self):
        ...
        self.assertEqual(len(records), 3)
        self.assertTrue(len(set([x['molecule_chembl_id'] for x in records]) ^ set(['CHEMBL6498', 'CHEMBL6499', 'CHEMBL6505'])) == 0)
    ...
    def test_get_pChembl_for_compound_and_target(self):
        ...
        activities = new_client.activity
        res = activities.filter(molecule_chembl_id="CHEMBL25", target_chembl_id="CHEMBL612545", pchembl_value__isnull=False)
        pchembls = [float(r['pchembl_value']) for r in res]
    ...
    def test_get_all_approved_drugs(self):
        ...
        self.assertTrue(1.74 < sum([float(d['molecule_properties']['alogp'] or 0) for d in approved_drugs if d['molecule_properties']]) / len(approved_drugs) < 1.76)
    ...
    def test_get_approved_drugs_for_lung_cancer(self):
        ...
        lung_cancer_mols = molecule.filter(molecule_chembl_id__in=[x['molecule_chembl_id'] for x in lung_cancer_ind])
        self.assertTrue(210 < len(lung_cancer_mols) < 220)
        self.assertTrue(set(['NICOTINE', 'SARACATINIB', 'BEMIPARIN']).issubset(set([l['pref_name'] for l in lung_cancer_mols])))
    ...
    def test_get_mols_with_no_ro5_violations(self):
        ...
        self.assertTrue(set(['GEMFIBROZIL', 'ANIROLAC', 'AZEPINDOLE']).issubset(set([n['pref_name'] for n in no_violations[:10000]])))
    ...
    def test_get_biotherapeutic_molecules(self):
        ...
        self.assertTrue(set(['LYPRESSIN', 'USTEKINUMAB', 'APICIDIN']).issubset(set([n['pref_name'] for n in biotherapeutics])))
    ...
    def test_get_all_natural_products(self):
        ...
        self.assertTrue('CC(=O)Oc1ccccc1C(=O)O' in [d['molecule_structures']['canonical_smiles'] for d in natural_products if d['molecule_structures']])
    ...
    def test_get_light_molecules(self):
        ...
        mean = sum([float(d['molecule_properties']['mw_freebase'] or 0) for d in light_molecules if d['molecule_properties']]) / len(light_molecules)
    ...
    def  test_get_ki_activities_for_herg(self):
        ...
        self.assertTrue(5300 < sum([float(x['standard_value'] or 0) for x in herg_activities]) / len(herg_activities) < 5400)
    ...
    def test_search_compound_in_unichem(self):
        ...
        self.assertTrue(set(['aspirin', 'CHEMBL25', 'SCHEMBL1353']).issubset(set([x['src_compound_id'] for x in ret[list(ret.keys())[0]]])))
